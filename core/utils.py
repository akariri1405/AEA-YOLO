...
def read_class_names(class_file_name):
    ...
    # If the input is a list (as is the case when using nargs='*'), use the first element.
    if isinstance(class_file_name, list):
        class_file_name = class_file_name[0]
    names = {}
...
def draw_bbox(image, bboxes, classes, show_label=True):
    ...
    bboxes: [xmin, ymin, xmax, ymax, probability, cls_id] format.
    """
...
def read_pb_return_tensors(graph, pb_file, return_elements):
    # Updated for TF2: use tf.io.gfile.GFile and tf.compat.v1.GraphDef.
...
def nms(bboxes, iou_threshold, sigma=0.3, method='nms'):
    ...
    bboxes: array with format (xmin, ymin, xmax, ymax, score, class).
    Supports both traditional NMS and soft-NMS.
    """
